# Links

A collection of references I am regularly going to.

## Walkthrough

- [Build Your Own Text Editor by snaptoken](https://viewsourcecode.org/snaptoken/kilo/01.setup.html)

## Terminal Controle Sequences

- [xterm](https://invisible-island.net/xterm/ctlseqs/ctlseqs.html)
- [Microsoft](https://learn.microsoft.com/en-us/windows/console/console-virtual-terminal-sequences)

## TUI

- [So you want to make a TUI…](https://p.janouch.name/article-tui.html)

## Unicode

- [The Absolute Minimum Every Software Developer Must Know About Unicode in 2023](https://tonsky.me/blog/unicode/)

## Text buffer implementation

- [VSCode Text Buffer Reimplementation](https://code.visualstudio.com/blogs/2018/03/23/text-buffer-reimplementation)
- [Text showdown: Gap Buffers vs Ropes](https://coredumped.dev/2023/08/09/text-showdown-gap-buffers-vs-ropes/)
- [Wikipedia's Gap buffer](https://en.wikipedia.org/wiki/Gap_buffer)
- [Wikipedia's Rope (data structure)](https://en.wikipedia.org/wiki/Rope_(data_structure))
- [Wikipedia's Piece table](https://en.wikipedia.org/wiki/Piece_table)
